...
No início, adicionei uma macro de jiggle de mouse, para que a máquina não trave nem entre em repouso durante uma ligação longa ou uma leitura longa. Primeiro o cursor só se mexia um pouco a cada 800 ms; agora ele percorre um círculo pixel a pixel. `M_JG_TOG` o inicia e o para deixando o cursor onde está: ao reiniciar, ele continua do mesmo ponto, com os mesmos ajustes. `M_JG_RUP` / `M_JG_RDN` aumentam e diminuem o raio (10 pixels por padrão, até um único pixel, que ainda move o mouse), e `M_JG_DUP` / `M_JG_DDN` alongam e encurtam a pausa entre dois pixels (1 segundo por padrão, até 1 ms, e depois ainda mais rápido, até 100 pixels por milissegundo). Nessas quatro teclas, um toque rápido, como ao digitar uma letra, muda o valor em 1, e segurar a tecla o muda de 10 em 10, cada vez mais rápido. `M_JG_CTG` escolhe onde acontece um clique esquerdo a cada volta: cada toque passa para a próxima das oito direções, norte, nordeste, leste, sudeste, sul, sudoeste, oeste, noroeste, com um bipe duplo um pouco mais agudo a cada vez, e depois desliga o clique com um bipe grave.
