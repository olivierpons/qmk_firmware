...
Early early I add mouse-jiggle macro, so di machine no go lock or idle during long call or long reading. Before, di cursor just dey shake small every 800ms; now e dey follow one circle, pixel by pixel. `M_JG_TOG` go start am and stop am, `M_JG_RUP` / `M_JG_RDN` go make di radius big or small (10 pixels by default, reach even one pixel wey still dey move di mouse), `M_JG_DUP` / `M_JG_DDN` go make di pause between two pixels long or short by one tenth of second (1 second by default; under 100 ms di step na 10 ms, under 10 ms e na 1 ms), and `M_JG_CTG` go on or off one left click every time di cursor reach di bottom right of di circle: very high beep mean on, very low beep mean off.
