...
Early early I add mouse-jiggle macro, so di machine no go lock or idle during long call or long reading. Before, di cursor just dey shake small every 800ms; now e dey follow one circle, pixel by pixel. `M_JG_TOG` go start am and stop am, and e go leave di cursor where e dey: next time wey you start am, e go continue from di same point, with di same settings. `M_JG_RUP` / `M_JG_RDN` go make di radius big or small (10 pixels by default, reach even one pixel wey still dey move di mouse), and `M_JG_DUP` / `M_JG_DDN` go make di pause between two pixels long or short (1 second by default, reach 1 ms, then even faster, reach 100 pixels for one millisecond). For dis four keys, one quick press, like when you dey type one letter, go change di value by 1, and if you hold di key e go change am by 10 at a time, faster and faster. `M_JG_CTG` go choose where one left click go happen for every round: every press go move to di next one for di eight directions, north, north east, east, south east, south, south west, west, north west, with double beep wey go dey small higher every time, then e go off di click with low beep.
