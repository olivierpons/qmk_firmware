...
Early on I added a mouse-jiggle macro, so the machine doesn't lock or go idle during a long call or a long read. At first the cursor just nudged itself every 800ms; now it follows a circle, pixel by pixel. `M_JG_TOG` starts and stops it and leaves the cursor where it is: the next start goes on from the same point, with the same settings. `M_JG_RUP` / `M_JG_RDN` grow and shrink the radius (10 pixels by default, down to a single pixel, which still moves the mouse), and `M_JG_DUP` / `M_JG_DDN` lengthen and shorten the pause between two pixels (1 second by default, down to 1 ms, then faster still, up to 100 pixels per millisecond). On these four keys, a quick tap, like typing a letter, changes the value by 1, and holding the key changes it by 10 at a time, faster and faster. `M_JG_CTG` picks where a left click happens on each turn: every press moves on to the next of the eight directions, north, north east, east, south east, south, south west, west, north west, with a double beep a little higher each time, then turns the click off with a low beep.
