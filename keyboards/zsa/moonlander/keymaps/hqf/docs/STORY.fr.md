...
Très tôt, j'ai ajouté une macro de jiggle de souris, pour que la machine ne se verrouille pas et ne passe pas en veille pendant un long appel ou une longue lecture. Au début, le curseur bougeait juste un peu toutes les 800 ms ; aujourd'hui, il suit un cercle pixel par pixel. `M_JG_TOG` le démarre et l'arrête en laissant le curseur là où il est : au démarrage suivant, il repart du même point, avec les mêmes réglages. `M_JG_RUP` / `M_JG_RDN` agrandissent et réduisent le rayon (10 pixels par défaut, jusqu'à un seul pixel, qui fait encore bouger la souris), et `M_JG_DUP` / `M_JG_DDN` allongent et raccourcissent la pause entre deux pixels (1 seconde par défaut, jusqu'à 1 ms, puis plus vite encore, jusqu'à 100 pixels par milliseconde). Sur ces quatre touches, un appui bref, comme pour taper une lettre, change la valeur de 1, et une touche maintenue la change de 10 à la fois, de plus en plus vite. `M_JG_CTG` choisit où se fait un clic gauche à chaque tour : chaque appui passe à la direction suivante parmi les huit, nord, nord-est, est, sud-est, sud, sud-ouest, ouest, nord-ouest, avec un double bip un peu plus aigu à chaque fois, puis coupe le clic avec un bip grave.
