...
Früh habe ich ein Mausbewegungs-Makro hinzugefügt, damit der Rechner während eines langen Anrufs oder einer langen Lektüre nicht sperrt oder in den Ruhezustand geht. Anfangs bewegte sich der Cursor nur alle 800 ms ein wenig; heute folgt er Pixel für Pixel einem Kreis. `M_JG_TOG` startet und stoppt ihn und lässt den Cursor, wo er ist: beim nächsten Start geht es am selben Punkt weiter, mit denselben Einstellungen. `M_JG_RUP` / `M_JG_RDN` vergrößern und verkleinern den Radius (standardmäßig 10 Pixel, bis hinunter zu einem einzigen Pixel, das die Maus immer noch bewegt), und `M_JG_DUP` / `M_JG_DDN` verlängern und verkürzen die Pause zwischen zwei Pixeln (standardmäßig 1 Sekunde, bis hinunter zu 1 ms, danach noch schneller, bis zu 100 Pixel pro Millisekunde). Auf diesen vier Tasten ändert ein kurzer Anschlag, wie beim Tippen eines Buchstabens, den Wert um 1, und eine gehaltene Taste ändert ihn um 10 auf einmal, immer schneller. `M_JG_CTG` wählt, wo bei jeder Runde ein Linksklick erfolgt: jeder Druck geht zur nächsten der acht Richtungen, Nord, Nordost, Ost, Südost, Süd, Südwest, West, Nordwest, mit einem Doppelton, der jedes Mal etwas höher ist, und schaltet den Klick danach mit einem tiefen Ton aus.
